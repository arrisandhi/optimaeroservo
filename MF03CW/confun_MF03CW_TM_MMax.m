function [cnlin, cnleq] = confun_MF03CW_TM_MMax(X0,BeamProp,BeamDef,BeamSeg,AeDef,GustDef,SimDef,Nmodes,RVnum,maxMomOri)
aRA = X0(1);
bRA = X0(2);
cRA = X0(3);
...
fprintf(1, 'Run LQR.\n');
CW = X0(4:5);
[u,x,y,Klqr] = main_LQR_MF03CW(ssLinRed,StateWORBRed,SimDef,GustDef,CW);
...
eigFlutter = max(real(eig(ssLinRed.a-ssLinRed.b(:,1:end-1)*Klqr)));
if eigFlutter > 0
    cFS = 1;
else
    cFS = 0;
