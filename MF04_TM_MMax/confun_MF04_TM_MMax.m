function [cnlin, cnleq] = confun_MF04_TM_MMax(X0,BeamProp,BeamDef,BeamSeg,AeDef,GustDef,SimDef,Nmodes,RVnum,maxMomOri)
aRA = X0(1);
bRA = X0(2);
cRA = X0(3);
dRA = X0(4);
...
RA = zeros(Ndivs,1);
for i = 1:Ndivs;
    RA(i) = aRA*NodeL(i)^3+bRA*NodeL(i).^2+cRA*NodeL(i)+dRA;
end
