function [cnlin, cnleq] = confun_MF05_TM_MMax(X0,BeamProp,BeamDef,BeamSeg,AeDef,GustDef,SimDef,Nmodes,RVnum,maxMomOri)
aRA = X0(1);
bRA = X0(2);
cRA = X0(3);
dRA = X0(4);
...
RA = zeros(Ndivs,1);
for i = 1:Ndivs;
    RA(i) = aRA*NodeL(i)^4+bRA*NodeL(i).^3+cRA*NodeL(i).^2+dRA*NodeL(i)+eRA;
end
